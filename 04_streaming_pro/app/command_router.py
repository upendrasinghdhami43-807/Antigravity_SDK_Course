...
class CommandRouter:
    ...
    def handle_command(self, cmd_input: str) -> bool:
        ...
        if cmd in ["exit", "quit"]:
            ...
            print_key_value("Avg Response Time", f"{stats['average_response_time']} sec")
            print_key_value("Avg First-Token Latency", f"{stats.get('average_latency', 0.0)} sec")
            print_key_value("Tokens/sec", str(stats.get('tokens_per_second', 0.0)))
            print_key_value("Chars/sec", str(stats.get('characters_per_second', 0.0)))
            ...
            if len(cmd_parts) > 1:
                ...
                current = self.chat_engine.settings_mgr.get("streaming", "ON")
                print(f"\nStreaming is currently: {current}\nUsage: /stream on|off\n")
            ...
            if confirm.lower() == 'y':
                ...
                self.chat_engine.stats_mgr.stats = {
                    ...
                    "average_latency": 0.0, "_total_latency": 0.0,
                    "_total_characters": 0, "characters_per_second": 0.0, "tokens_per_second": 0.0
                }
