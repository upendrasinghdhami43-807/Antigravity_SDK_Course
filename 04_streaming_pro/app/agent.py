...
class Agent:
    ...
    async def generate_stream(self, prompt: str):
        """Asynchronously yields chunks from the Gemini API."""
        if not self.client:
            yield "Error: Gemini API key is not configured."
            return
            
        try:
            # We use the aio version of the client for async streaming
            ...
            clean_msg = _clean_error(e)
            logger.error(f"Error generating stream: {clean_msg}")
            yield f"\n[Error: {clean_msg}]"
